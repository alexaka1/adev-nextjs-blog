...
Looking at [.NET 8's servicing history](https://github.com/dotnet/runtime/pulls?q=is%3Amerged+base%3Arelease%2F8.0+label%3AServicing-approved), the team has backported hundreds of fixes over the LTS lifecycle. Here are some examples of non-security fixes that made it through:

### Crashes and Runtime Stability

- **[Debugger crash fix](https://github.com/dotnet/runtime/pull/110651)** — A crash while debugging apps due to a null reference. Merged December 2024, shipped in .NET 8.0.13.

- **[GC freelist crash](https://github.com/dotnet/runtime/pull/104992)** — A garbage collection issue that could cause crashes when using `GC.RegisterForFullGCNotification`. Customer-reported, merged July 2024, shipped in .NET 8.0.10.

- **[Android method compilation crash](https://github.com/dotnet/runtime/pull/109511)** — A Mono JIT crash affecting Android applications. Customer-reported, merged November 2024, shipped in .NET 8.0.12.

### Regressions from Previous Versions

- **[System.Text.Json source generator regression](https://github.com/dotnet/runtime/pull/98082)** — Properties using C# reserved keywords as identifiers caused the source generator to produce uncompilable code. This was a regression from .NET 7, merged February 2024, shipped in .NET 8.0.3.
...
1. Workarounds are acceptable for 12+ months
2. Developer experience issues (like compiler warnings) aren't urgent
3. Monthly patch cycles are fast enough for regressions

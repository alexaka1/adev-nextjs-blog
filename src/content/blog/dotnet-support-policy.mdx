...
- **[AssemblyLoadContext.Unload crash](https://github.com/dotnet/runtime/pull/91346)** — When calling `AssemblyLoadContext.Unload` twice, the runtime would crash with a FailFast. This was backported because it caused application crashes.

- **[GC freelist crash](https://github.com/dotnet/runtime/pull/104992)** — A garbage collection issue that could cause crashes under specific conditions.

- **[Android method compilation crash](https://github.com/dotnet/runtime/pull/109511)** — A Mono JIT crash affecting Android applications.

### Regressions from Previous Versions

- **[System.Text.Json source generator regression](https://github.com/dotnet/runtime/pull/98082)** — Properties using C# reserved keywords as identifiers caused the source generator to produce uncompilable code. This was a regression from .NET 7 and was backported.
